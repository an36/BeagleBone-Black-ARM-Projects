@ Counter program on interrupt basis using a Button.
@ This program uses the button on GPIO1_29 to turn the timer (Timer2) and LEDs on on 
@ an interrupt basis.  When the button is pushed for the second time the program 
@ will turn off the timer and LEDs while turning the talker (UART5) which will say 
@ the specified number (in HEX) by the timer once it has been stopped on an interrupt basis.
@ Uses: R0-R11, GPIO1_29, UART5 and Timer2
@ Abdullah Almarzouq February 2020
...
_start:
		...
		@Clearing bits 21-24 of GPIO1
		MOV R2,#0x01E00000			@Value to clear bits 21-24 off with GPIO1_CLEARDATAOUT
		STR R2,[R1,#0x190]			@Write to GPIO1_CLEARDATAOUT register to clear bit 21 on register
		
		@Set bits on GPIO1 as Output Enable
		LDR R6,[R1,#0x0134]			@Read current GPIO1 output enable register
		MOV R7,#0xFE1FFFFF			@Word to enable bits 21-24 of GPIO1 as output
		AND R6,R7,R6				@Clear bits 21-24 (MODIFY)
		STR R6,[R1,#0x0134]			@Write to GPIO1 output enable register
		...
		@Initialize Timer2 registers with count value
		MOV R0,#0
		LDR R0,=0x48040000			@Base address for Timer2 registers
		MOV R2,#0x1					@Value to reset Timer2
		STR R2,[R0,#0x10]			@Write to Timer2 CFG register
		LDR R2,=0x00000000			@start count value at zero seconds
		STR R2,[R0,#0x40]			@Write to Timer2 TLDR load register (Reload value)
		STR R2,[R0,#0x3C]			@Write to Timer2 TCRR count register
		...
		STR R1,[R0,#0x0C]			@Write to UART5_LCR
		MOV R1,#0x0000				@Value to disable THRIT bit and MODDEMSTSIT bit
		...
		MOV R11,#0					@R11 checks if the command to chenage the voice of the talker has already been sent or not
...
BUTTON_SVC:
	...
	Delay2: @this delay loop fixes the problem when the button is acting as if it was being pushed twice
		...
		CMP R10,#1					@If R10=1 then button has been pushed the first
		BPL OFF						@if R10=1 then go to branch off to turn off leds and timer
		MOV R10,#1					@Let R10=1 to indicate that the button has been pushed for a first time		
		...
		MOV R1,#0x03				@Value to auto reload timer and start
		LDR R0,=0x48040038			@Address of Timer2 TCLR register
		STR R1,[R0]					@Write to TCLR register
		
		@Toggle LEDs on
		MOV R3,#0x01E00000			@Value to set bits 21 to 24 to one
		LDR R0,=0x4804C194			@Load address of GPIO1_SETDATAOUT register
		STR R3,[R0]					@Use GPIO1_SETDATAOUT register to light up USR LEDs
		...
		MOV R1,#0x0000				@Value to disable THR, MODEM and CTS interrupts on IER_UART5
...
VADERV:@Branch to change the voice to Darth Vader
		...
		BNE PASS_ON					@If counter value greater than or equal to zero then go back to wait loop
		...
		MOV R11,#1					@Let R11=1 to indicate that the voice has been changed already
...
TLKR_SVC:@Branch to send characters to UART5
		...
		BNE PASS_ON					@Greater than or equal to zero then go to PASS_ON
		...
		MOV R11,#0					@Indicate that the program has been to TLKR_SVC
		
		B PASS_ON
		
OFF:@Branch to turn off LEDs and Timer2
		MOV R10,#0					@Set button checker (R10) back to zero
		MOV R11,#0					@Set R11=0 to be able to send command again to the talker device

		MOV R1,#0x00				@Value to stop timer
		LDR R0,=0x48040000			@Address of Timer2 TCLR register
		STR R1,[R0,#0x038]			@Write to TCLR register
		LDR R6,[R0,#0x3C]			@Load value of timer in TCRR register
		
		MOV R7,#8					@Initialize loop counter (for STR branch loop)
		LDR R1,=MESSAGE				@Load th address of the ascii message that will be sent to the talker
		ADD R1,R1,#10				@Point the address at the end of the ascii string
		MOV R4,#0x0000000F			@R4 hold the value to AND each number in the hex value of the timer
		MOV R5,#0					@R5 initialized to 0 because it is going to be used to shift the ANDed value inside the loop
		MOV R9,#0x0					@R9 is 0 and it is going to be used for the ADDing and Shifting commands
		
CNV:
		AND R8,R4,R6				@AND a number within the timer value to be stored alone in the ascii message string
		ADD R4,R9,R4,LSL #4			@Shift the ANDing word four bits to the left to be used to AND the next number
		ADD R8,R9,R8,LSR R5			@Shift the ANDed value a x bits to the right (first value x=0, second value x=4, third value x=8 and so on...)
		ADD R5,R5,#4				@ADDing four to R5 to be used for the right shift of the ANDed value
		B HtoA
STR:
		STRB R8,[R1]				@Storing the ANDed value inside the message ascii string
		SUBS R1,R1,#1				@Decrement the message address to point at the preceeding value to be replaced with the ANDed timer value
		SUBS R7,#1					@Decrement the loop counter
		BNE CNV						
		
		LDR R1,=0x00000000			@Value to reset Timer2 TCRR
		STR R1,[R0,#0x3C]			@Store reset value to timer2 TCRR register
		
		@Toggle LEDs off
		MOV R3,#0x01E00000			@Value to clear bits 21-24 off with GPIO1_CLEARDATAOUT
		LDR R0,=0x4804C190			@ load address of GPIO1_CLEARDATAOUT
		STR R3,[R0]					@Write to GPIO1_CLEARDATAOUT register to clear bit 21 on register
		
		LDR R0,=0x481AA000			@Base address of UART5
		MOV R1,#0x000A				@Value to enable THRIT bit and MODDEMSTSIT bit
		STR R1,[R0,#0x04]			@Write to IER_UART5 register
		
		B PASS_ON
		
HtoA:@Branch to convert HEX values to ASCII
	CMP R8,#0xA						@Compare to check if hex value is a letter (A-F) or a number (0-9)
	BMI NUM							@If HEX value is a number then jump to branch NUM
	ADD R8,R8,#0x37					@Else just add 0x37 to the HEX value to convert it to ASCII character
	B STR							@Jump to STR branch to store ASCII character in MESSAGE
NUM:@Branch to convert HEX numbers to ASCII characters
	ADD R8,R8,#0x30					@If HEX value is a number then just add 0x30
	B STR							@Jump back to STR branch to store ASCII character is MESSAGE
...
VADER:
.byte 0x01
.ascii "0O"					@Command to change the speaker voice to VADER
.byte 0x01
.ascii "15C"					@Command to let the speaker pronounce the MESSAGE letter by letter
.byte 0x01
.ascii "8S"					@Command to increase the speed of the speaker 
.byte 0x01
.ascii "50P"					@Command to increase the speed of the speaker 
.byte 0x01
